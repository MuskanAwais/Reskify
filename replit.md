...
- June 19, 2025. Universal Template Consistency & Card Spacing Fix - Fixed Construction Control Risk Matrix card spacing issues across all sample files and PDF generation endpoints. Implemented corrected positioning calculations with equal 20px horizontal and 15px vertical gaps between all four cards (A, B, C, D). Created unified pdf-generator-corrected.ts ensuring all sample files use latest template with proper card dimensions and content positioning. Reduced file sizes from 211KB to 5-6KB while maintaining professional quality. All sample projects (140, 141, 142, 143) now generate consistently with corrected spacing.
