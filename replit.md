...
- June 26, 2025. Safety Library Admin-Only Management & Clean Implementation - Completely restructured Safety Library with admin-only access controls. Cleared all existing documents from database and simplified to single "Code of Practices" folder structure. Implemented strict admin-only add/delete functionality - only app administrators can upload or remove documents. Removed download options for all users, replaced with view-only access. Fixed PDF viewing to display actual uploaded files instead of Google search redirects. Added proper document deletion API endpoint with admin authentication checks. Non-admin users can only view documents through popup viewer with proper PDF content display. System now enforces secure document management with clear separation between admin capabilities and user access rights.
