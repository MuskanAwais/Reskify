...
- June 30, 2025. Analytics Data Consistency & Compliance Fix - Fixed analytics data consistency issues and compliance metric accuracy. Analytics endpoint now uses real database data matching My SWMS information structure. Compliance scores set to 100% across all metrics since every generated SWMS follows Australian WHS standards. Removed misleading "Performance" metric card and replaced with meaningful "Draft Documents" counter. Analytics now correctly shows system-wide statistics (7 total docs, 5 active) while My SWMS shows user-specific data (3 user docs). Compliance trend chart displays consistent 100% scores reflecting true Australian standards compliance. System now provides accurate, meaningful analytics aligned with actual platform functionality.
